...
export default {
  ...
  async asyncData () {
    ...
    const nexts = []
    const nextDocs = await firebase.firestore().collection('nexts').get()
  ...
  methods: {
    ...
    async clickRegist () {
      this.loading = true
      const nextsRef = firebase.firestore().collection('nexts')
    ...
    async clickDelete (id) {
      this.loading = true
      this.dialog = true
      await firebase.firestore().collection('nexts').doc(id).delete()
